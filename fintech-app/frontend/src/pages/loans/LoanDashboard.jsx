...
function LoanDashboard({ loan, loanForm, transactions, onPayLoan, onRefresh }) {
  ...
  const progress = calculateLoanProgress(loan.disbursedAmount, loan.principalAmountLeft)
    ...
    <div className="loan-flow">
      ...
      <div className="loan-dashboard-header">
        ...
        <div className="loan-dashboard-header__actions">
          <button type="button" className="auth-button auth-button--secondary" onClick={onRefresh}>
            ↻ Refresh
          </button>
          <button type="button" className="auth-button loan-btn-primary" onClick={onPayLoan}>
            Make Payment →
          </button>
        </div>
      </div>
      ...
      <div className="dashboard-card loan-progress-card">
        ...
        <div className="loan-progress-bar">
          <div className="loan-progress-bar__fill" style={{ width: `${progress}%` }} />
        </div>
        <div className="loan-progress-card__stats">
          ...
          <div>
            ...
            <strong>{paidEmis} / {loan.disbursedDuration}</strong>
          </div>
          <div>
        ...
        <div className="loan-overview-grid">
          <div className="dashboard-card loan-detail-card">
            ...
            <dl className="loan-detail-list">
              <div><dt>Loan ID</dt><dd>{loan._id.slice(-8).toUpperCase()}</dd></div>
              <div><dt>Disbursed Amount</dt><dd>{formatCurrency(loan.disbursedAmount)}</dd></div>
              <div><dt>Interest Rate</dt><dd>{loan.disbursedInterest}% p.a.</dd></div>
              <div><dt>Tenure</dt><dd>{loan.disbursedDuration} months</dd></div>
              <div><dt>Monthly EMI</dt><dd>{formatCurrency(loan.emiAmount)}</dd></div>
              <div><dt>Principal Remaining</dt><dd>{formatCurrency(loan.principalAmountLeft)}</dd></div>
              <div><dt>Status</dt><dd><span className="loan-badge loan-badge--paid">Active</span></dd></div>
            </dl>
          </div>
          ...
          {loanForm && (
            <div className="dashboard-card loan-detail-card">
              ...
              <dl className="loan-detail-list">
                <div><dt>Name</dt><dd>{loanForm.applicantName}</dd></div>
                <div><dt>Email</dt><dd>{loanForm.applicantEmail}</dd></div>
                <div><dt>Phone</dt><dd>{loanForm.applicantPhone}</dd></div>
                <div><dt>PAN</dt><dd>{loanForm.panNumber}</dd></div>
                <div><dt>Purpose</dt><dd>{loanForm.loanPurpose}</dd></div>
                <div><dt>Credit Score</dt><dd>{loanForm.creditScore}</dd></div>
              </dl>
            </div>
          )}
          ...
          <div className="dashboard-card loan-detail-card loan-detail-card--highlight">
            <h4>Quick Pay</h4>
            <p>Pay your current month EMI of <strong>{formatCurrency(loan.emiAmount)}</strong></p>
            <button type="button" className="auth-button loan-btn-primary" onClick={onPayLoan}>
              Pay Now →
            </button>
          </div>
        ...
        <div className="dashboard-card">
          <div className="expenses-table-wrap">
            <table className="expenses-table loan-schedule-table">
              ...
              <tbody>
                {schedule.map((row) => (
                  <tr key={row.month} className={row.status === 'paid' ? 'loan-row--paid' : ''}>
                    <td>{row.month}</td>
                  ...
                  {[...transactions]
                    ...
                    .map((txn) => (
                      <tr key={txn._id}>
                        ...
                        <td>
                          <span className={`loan-badge loan-badge--${txn.transactionType === 'EMI' ? 'paid' : 'prepay'}`}>
                            {txn.transactionType}
                          </span>
                        </td>
                        <td>{formatCurrency(txn.transactionAmount)}</td>
                        <td className="loan-payment-id">{txn.razorpayPaymentId}</td>
                      </tr>
                    ))}
                </tbody>
              </table>
